...
def subset_train_val_test_loader(dataset, collate_fn=default_collate,
                              batch_size=64,random_seed = 2, val_ratio=0.1, test_ratio=0.1, 
...
class CGData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        crystal_graph = np.load(os.path.join(self.root_dir,
                                                   cif_id+'.npz'))
        atom_fea = crystal_graph['atom_fea']
        nbr_fea = crystal_graph['nbr_fea']
        nbr_fea_idx = crystal_graph['nbr_fea_idx']                        
